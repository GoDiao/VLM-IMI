...
def generate_and_save_descriptions(image_dir, output_folder):
    ...
    model = LlavaForConditionalGeneration.from_pretrained("/data/sunxr/LLM-Diff/pretrained/llava",
                                                          torch_dtype=torch.float16,
                                                          low_cpu_mem_usage=True,
                                                          attn_implementation="flash_attention_2").to(0)
    processor = AutoProcessor.from_pretrained("/data/sunxr/LLM-Diff/pretrained/llava")
...
image_directory = '/data/sunxr/LLM-Diff/data/high'
output_directory = '/data/sunxr/LLM-Diff/data/txt'
